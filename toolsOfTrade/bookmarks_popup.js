...
var currentParentFolder = BOOKMARKS_BAR;
...
function onGetChildrenSuccess(children) {
    ...
    if (!loadedFolders) {
        if (currentParentFolder != OTHER_BOOKMARKS_FOLDER) {
            currentParentFolder += 1;
            console.log(`The current folder is: ${currentParentFolder}`);
        }
        else {
            loadedFolders = true;
